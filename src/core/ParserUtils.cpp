...
namespace ParserUtils {
	...
	YAML::Node ExtractChildren(const YAML::Node& mapNode) {
		// If this is a one-entry map (for example {random: [...]}), return its body.
		...
		// Fallback: first sequence child anywhere.
		for (auto it = mapNode.begin(); it != mapNode.end(); ++it) {
	...
	Node* ParseNode(const YAML::Node& yamlNode, Context& ctx) {
		...
		if (yamlNode.IsScalar()) {
			...
			// Peel off the inner map or sequence.
			...
			if (def.IsMap()) {
				...
				// A sequence or scalar already contains the children payload.
				children = def;
			}
			...
			if (key == "sound" || key == "delay") {
				...
				// Iterate cases, but recurse only on the inner sound node.
