...
void AudioManager::Update(float /*dt*/)
{
	...
	while (inQueue->pop(cmd))
	{
		...
		case CommandType::SetTag:
			SetTag(cmd);
			break;
		case CommandType::SetTransient:
			SetTransient(cmd);
			break;
		case CommandType::ClearTag:
			ClearTag(cmd);
			break;
		...
		case CommandType::ClearValue:
			ClearValue(cmd);
			break;
		case CommandType::LoadBehaviors:
			LoadBehaviorsFromFolder(cmd);
			break;
		case CommandType::Shutdown:
			Shutdown();
			break;
		case CommandType::AssetPath:
			SetAssetPath(cmd);
			break;
		case CommandType::ClearEntity:
			ClearEntity(cmd);
			break;
		default:
			LogMessage("UNKNOWN COMMAND: " + cmd.GetTypeName(), LogCategory::AudioManager, LogLevel::Debug);
		}
	}
		...
		for (auto &inst : ed.GetBehaviors())
			for (auto &v : inst.GetVoices())
			{
				if (!v.buffer)
					continue;
				if (v.buffer->Empty())
					continue; // TODO: buffer not ready. but this would offset our start, right? not sure how to properly handle this tbh...?
...
void AudioManager::DebugPrintState()
{
	for (auto &e : entities)
	{
		...
		for (auto &val : e.second.GetValues().GetAllValues())
		{
			...
			{
				LogMessage("    - " + n + ": " + "(" + std::to_string(vv.x) + ", " + std::to_string(vv.y) + ", " + std::to_string(vv.z) + ")", LogCategory::AudioManager, LogLevel::Debug);
			}
		}
